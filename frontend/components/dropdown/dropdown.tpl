...
{block 'dropdown_content'}{strip}
    <{$tag|default:"div"} class="{$component} {$classes}" {cattr list=$attributes}>
        {foreach $buttons as $button}
            {if is_array($button)}
                {component 'button' params=$button bmods="{$bmods} {$button.bmods}"}
            {else}
                {$button}
            {/if} 
        {/foreach}
        ...
        {if $toggler}
            {if is_array($toggler)}
                ...
                {component 'button' params=$toggler}
            {else}
                {$toggler}
            {/if}
        {else}
                            ...
                            <a class="dropdown-item {$item.classes} " href="{$item.url}">
                                ...
                                {if $item.count}
                                    {if is_array($item.count)}
                                        {component "badge" params=$item.count}
                                    {else}
                                        {component "badge" 
                                            text        = $item.count 
                                ...
                                <span class="dropdown-item-text {$item.classes} ">
                                    ...
                                    {if $item.badge}
                                        {if is_array($item.badge)}
                                            {component "badge" prrams=$item.badge}
                                        {else}
                                            {component "badge" text=$item.badge bmods="primary"}
                                        {/if}                    
                                    {/if}                                    
                                </span>
                            {/if}                            
